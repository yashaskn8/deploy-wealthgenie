...
export const CURRENT_FY: string = getCurrentFiscalYear();

const defineSlabs = (slabs: Slab[]): readonly Slab[] => Object.freeze(slabs.map(slab => Object.freeze({ ...slab })));

export const TAX_SLABS_BY_FY: Readonly<Record<string, SlabsForFY>> = Object.freeze({
...
export function getTaxSlabsForFY(fiscalYear: string = CURRENT_FY): SlabsForFY {
 ...
 */
export function isFYVerified(fiscalYear: string = CURRENT_FY): boolean {
  const entry = TAX_SLABS_BY_FY[fiscalYear];
  return entry ? entry.verified === true : false;
}

function getRegimeSlabs(regime: 'new' | 'old', fiscalYear: string = CURRENT_FY): readonly Slab[] {
 ...
 */
function calculateFromSlabs(taxableIncome: number, slabs: readonly Slab[]): number {
  let tax = 0;
 ...
 */
function computeSurcharge(taxBeforeSurcharge: number, taxableIncome: number, regime: 'new' | 'old'): number {
 ...
 */
function computeMarginalRelief(
  baseTax: number,
  surcharge: number,
  taxableIncome: number,
  regime: 'new' | 'old',
...
export function computeTax(
  ...
  let safeRegime: 'new' | 'old' = (regime === 'new' || regime === 'old') ? regime : 'new';
 ...
 */
export function computeTaxWithDeductions(
  annualIncome: number,
  regime: 'new' | 'old',
  deductions: Deductions = {},
  incomeSource: string = 'salary',
 ...
 */
export function getEffectiveMarginalRate(
  annualIncome: number,
  regime: 'new' | 'old' = 'new',
  deductions: Deductions = {},
  incomeSource: string = 'salary',
